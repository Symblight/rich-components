...
export class ChxAttachments extends LitElement {
    ...
    get fileInputElement() {
        return /** @type {HTMLInputElement} */ (
            this.renderRoot?.querySelector(".attachments__input")
        );
    }
    ...
    buildDefaultCard(file) {
        if (this._cardTemplate) {
            const clone = /** @type {DocumentFragment} */ (
                this._cardTemplate.content.cloneNode(true)
            );
            const root = /** @type {(HTMLElement & {file?: File}) | null} */ (
     ...
     * see .claude/plans/attachments.spec.md.
     */
    handleSlotChange = () => {
        this.dispatchEvent(
